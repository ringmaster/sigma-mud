        ...
        if speaker.engaged.combatant1 == engagee or speaker.engaged.combatant2 == engagee:
            ...
            for co in speaker.combats:
                if co.combatant1 == engagee or co.combatant2 == engagee:
                    ...
                    report(ROOM, "$actor $verb " + pronoun_possessive[speaker.gender] + " attention toward $direct!",
                           speaker, ("turn", "turns"), engagee)
                    speaker.engaged = co
...
def advance(data):
    ...
    if not pwr:
        pwr = preferred_range[
            'bare handed' if not speaker.equipped_weapon else speaker.equipped_weapon[0].weapon.weapon_type]

    if pwr >= speaker.engaged.range:
        speaker.send_line("You can't advance, you are already at " + val2txt(speaker.engaged.range, range_match_val,
                                                                             range_match_txt) + " range with " + (
                          speaker.engaged.combatant1.name if speaker.engaged.combatant1 != speaker else speaker.engaged.combatant2.name) + "!")
        return

    speaker.engaged.set_override_range(speaker, pwr)
    speaker.engaged.set_action(speaker, COMBAT_ACTION_ADVANCING)

    speaker.send_line("You will attempt to advance upon " + (
    speaker.engaged.combatant1.name if speaker.engaged.combatant1 != speaker else speaker.engaged.combatant2.name) + " at the next opportunity.")
...
def withdraw(data):
    ...
    if pwr <= speaker.engaged.range:
        speaker.send_line("You can't withdraw, you are already at " + val2txt(speaker.engaged.range, range_match_val,
                                                                              range_match_txt) + " range with " + (
                          speaker.engaged.combatant1.name if speaker.engaged.combatant1 != speaker else speaker.engaged.combatant2.name) + "!")
        return
    ...
    speaker.send_line("You will attempt to withdraw from " + (
    speaker.engaged.combatant1.name if speaker.engaged.combatant1 != speaker else speaker.engaged.combatant2.name) + " at the next opportunity.")
